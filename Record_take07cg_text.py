...
def upscale_image(image, target_size=(1000, 1000)):
    ...
    upscaled_image_cv = cv2.cvtColor(np.array(upscaled_image), cv2.COLOR_RGB2BGR)
    return upscaled_image_cv

def extract_text(image):
    # Convert the image to RGB (pytesseract requires this format)
    rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    
    # Configure tesseract to use the Cyrillic language
